...
\COPY COURSE_NAME_GEN FROM '/home/alena/Yandex.Disk/aqo/Beginer_courses.csv' WITH (FORMAT csv, DELIMITER ',', QUOTE '"');
\COPY COURSE_NAME_GEN FROM '/home/alena/Yandex.Disk/aqo/Intermediate_courses.csv' WITH (FORMAT csv, DELIMITER ',', QUOTE '"');
\COPY COURSE_NAME_GEN FROM '/home/alena/Yandex.Disk/aqo/Expert_courses.csv' WITH (FORMAT csv, DELIMITER ',', QUOTE '"');
...
begin
  for i in 1..length loop
  	...
  	select sno, sgroup into st, gr from student_frgn where sno not in (select sno from score where cno = c) order by random() limit 1;
